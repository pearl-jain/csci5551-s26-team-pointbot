...
class PointBot:
    ...
    def solve(self, t_cam_robot):
        ...
        scalar = np.dot((plane_point - origin_rob), plane_normal) / dot_line_plane

        intersect = origin_rob + scalar * ray_rob

        # Find intersection by setting eq = 0 (origin_z + t * dir_z = 0)
        # t = 1 - origin_rob[1] / ray_rob[1]
        # intersection = origin_rob + t * ray_rob
        intersection_cam = (np.linalg.inv(t_cam_robot) @ np.append(intersect, 1))[:3]

        return p_tip_cam, ray_cam, origin_rob, ray_rob, intersect, intersection_cam
    ...
    def visualize(self, frame, p_tip_cam, ray_cam, intersection_cam, lm):
        h, w = frame.shape[:2]
        ...
        #     cv2.circle(frame, p0, 5, (255,0,0), 10)
        #     cv2.circle(frame, p1, 5, (0,255, 255), 20)
        #     cv2.line(frame, p0, p1, (0, 255, 255), 2)

        point = [0,1,0]
        print(p_tip_cam, intersection_cam, point)
        print(p0, p_int, self.proj_3d_2d(point))
        ...
        # X, Y, Z, _ = self.depth[v, u]
        # p = np.array([X, Y, Z])


        # lm_pcd = o3d.geometry.PointCloud()
        # lm_pcd.points = o3d.utility.Vector3dVector(p)
        # lm_pcd.paint_uniform_color([1, 0, 0])
        ...
        y = 100
        x = intersection_cam[0]
        z = intersection_cam[2]
        pts = []
        for i in range(25):
            pt = [x +i, y, z+i]            
            pts.append(pt)
        ...
        o3d.visualization.draw_geometries([frame_pcd, line_set, pcd])
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if len(self.frame_buffer) > TARGET_FRAMES:
                    ...
                    # tip_cam, ray_cam, tip_rob, ray_rob = self.solve(t_cam_robot)
                    frame = self.visualize(frame, tip_cam, ray_cam, inter_cam, lm.landmark[8])
